...
for (const colorScheme of ["light", "dark"] as const) {
  for (const width of [390, 1440]) {
    test(`pause visibly rewinds into the matching static opening at ${width}px in ${colorScheme}`, async ({
      page,
    }) => {
      ...
      const opening = await page.screenshot({
        clip,
        path: `artifacts/opening-pixels-${width}-${colorScheme}.png`,
      });
      ...
      const after = await sharp(
        await page.screenshot({
          clip,
          path: `artifacts/paused-pixels-${width}-${colorScheme}.png`,
        }),
      )
        .removeAlpha()
        .raw()
      ...
      await expect(
        section.locator(".story-chapters li").first(),
      ).toHaveAttribute("aria-current", "step");
      ...
      expect(await section.evaluate((el) => el.clientHeight)).toBeLessThan(
        1100,
      );
      await expect
        .poll(() =>
      ...
      await page.screenshot({
        path: `artifacts/static-pause-${width}-${colorScheme}.png`,
      });
